...
Pri spusteni ze zdrojaku je soubor v rootu projektu: `tagy.txt`.
Pri EXE distribuci je standardne vedle `PhotosTagger.exe`.
Pokud je EXE slozka jen pro cteni, fallback je `%LOCALAPPDATA%\PhotosTagger\tagy.txt`.
